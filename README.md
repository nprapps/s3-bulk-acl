...
**TKTK: Describe s3-bulk-acl here.**
...
View the [sample copy spreadsheet](https://docs.google.com/spreadsheet/pub?key=1OD44eQfkubnkNryvHEfpvh_uQcnw1W8GyPg2PUg-i_U#gid=0).
...
def read_my_google_doc():
    ...
    get_document('1OD44eQfkubnkNryvHEfpvh_uQcnw1W8GyPg2PUg-i_U', file_path)
    data = Copy(file_path)
...
```
* * * * * ubuntu bash /home/ubuntu/apps/s3_bulk_acl/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test
```
...
|--------|------|-----|-----|
|s3-bulk-acl|tweet|`location`||
|s3-bulk-acl|facebook|`location`||
|s3-bulk-acl|email|`location`||
|s3-bulk-acl|new-comment||
|s3-bulk-acl|open-share-discuss||
|s3-bulk-acl|close-share-discuss||
|s3-bulk-acl|summary-copied||
|s3-bulk-acl|featured-tweet-action|`action`|
|s3-bulk-acl|featured-facebook-action|`action`|
